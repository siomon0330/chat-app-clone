...
class ContactsTableViewController: UITableViewController, UISearchResultsUpdating, UserTableViewCellDelegate {
    ...
    //MARK: IBActions
    @objc func inviteUserButtonPressed(){
    ...
    func setupButtons(){
        if isGroup{
            ...
            //private chat
            let inviteButton = UIBarButtonItem(image: UIImage(named:"invite"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(self.inviteUserButtonPressed))
            let searchButton = UIBarButtonItem(image: UIImage(named:"nearMe"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(self.searchNearByButtonPressed))
